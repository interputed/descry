...
int main(int argc, char **argv)
{
    ...
    try {
        ...
        std::cerr << "Correct Usage: " << argv[0] << "<full input image path>"
                  << std::endl;
        return 1;
    }
...
std::string OutputValidation(const std::string input_file)
{
    ...
    output_file_mod.insert (
        output_file_mod.length() - input_path.extension().string().length(),
        OUTPUT_APPEND
    );
    ...
    if (boost::filesystem::exists(output_path)) {
        ...
        if (!boost::filesystem::remove(output_path)) {
            throw std::invalid_argument(
                std::string("Failed to remove existing output file.")
            );
        }
    }
